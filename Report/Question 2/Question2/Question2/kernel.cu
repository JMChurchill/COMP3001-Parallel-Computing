...
__global__ void mmm_implementation1(float* C, float* A, float* B) {
	for (int i = 0; i < N; i++)
		for (int j = 0; j < N; j++)
			for (int k = 0; k < N; k++)
...
__global__ void mmm_implementation2() {
...
__global__ void mmm_implementation3(float* C, float* A, float* B) {
	float tmp = 0.0;
...
__global__ void mmm_implementation4(float* C, float* A, float* B) {
...
//---------------------------create GPU arrays------------------------------------------
	...
	for (int it = 0; it < TIMES_TO_RUN; it++) {
		...
		mmm_implementation1 << <dimGrid, dimBlock >> > (C_d, A_d, B_d);
